...
void sm2Jiemi(char **sm2_param, int type, int point_bit_length , char *miwen , char *output){
    ...
    sm2_decrypt(ecp, &message_data);
    printf("解密后是 decrypt: len: %lu\n%s\n\n\n\n\n", strlen(message_data.decrypt), message_data.decrypt);
    
    memcpy(output, message_data.decrypt, strlen(message_data.decrypt));
...
//void sm2Jiemi(char **sm2_param, int type, int point_bit_length , char *miwen){
//    ec_param *ecp;
//    sm2_ec_key *key_B;
//    message_st message_data;
//    //ecp的开辟空间p a b n
